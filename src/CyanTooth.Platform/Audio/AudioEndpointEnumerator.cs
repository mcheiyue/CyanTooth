...
public class AudioEndpointEnumerator : IDisposable
{
    ...
    public IReadOnlyList<AudioEndpointInfo> GetBluetoothAudioEndpoints()
    {
        ...
        try
        {
            ...
            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: total audio endpoints={count}");
                ...
                try
                {
                    ...
                    for (uint j = 0; j < connectorCount; j++)
                    {
                        try
                        {
                            ...
                            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: found BT device, connectedDeviceId={connectedDeviceId}");
                            ...
                            if (connectedDevice != null)
                            {
                                ...
                                {
                                    CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: failed to activate IKsControl: {ex.Message}");
                                }
                            }
                            ...
                            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: added endpoint Name={friendlyName}, HasKsControl={ksControl != null}");
                        }
                        catch (Exception ex)
                        {
                            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: connector error: {ex.Message}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: device error: {ex.Message}");
                }
            }
        }
        catch (COMException ex)
        {
            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: COM error: {ex.Message}");
        }

        CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"GetBluetoothAudioEndpoints: returning {_endpoints.Count} endpoints");
        return _endpoints;
    }
    ...
    public IEnumerable<AudioEndpointInfo> FindEndpointsByMacAddress(ulong macAddress, string? deviceName)
    {
        ...
        CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: looking for MAC={macHex}, DeviceName={deviceName ?? "null"}");
        
        var endpoints = GetBluetoothAudioEndpoints();
        
        CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: found {endpoints.Count} BT endpoints total");
        
        foreach (var ep in endpoints)
        {
            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: endpoint Name={ep.FriendlyName}, ConnectedDeviceId={ep.ConnectedDeviceId ?? "null"}");
        }
        ...
        CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: {matchedByMac.Count} matched by MAC");
        ...
        if (!string.IsNullOrEmpty(deviceName))
        {
            ...
            CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: {matchedByName.Count} additional Hands-Free matched by name");
            
            matchedByMac.AddRange(matchedByName);
        }
        
        CyanTooth.Platform.CyanTooth.Platform.Helpers.DebugLogger.Log($"FindEndpointsByMacAddress: {matchedByMac.Count} total matched");
