...
public static class BluetoothServiceStateApi
{
    ...
    {
        DebugLogger.Log($"BluetoothServiceStateApi.DisconnectAudioDevice: MAC={bluetoothAddress:X12}");
        ...
        DebugLogger.Log($"BluetoothServiceStateApi: A2DP disable result={resultA2dp}");
        ...
        DebugLogger.Log($"BluetoothServiceStateApi: HFP disable result={resultHfp}");
        ...
        DebugLogger.Log($"BluetoothServiceStateApi: HSP disable result={resultHsp}");
        if (resultHsp == 0) anySuccess = true;

        DebugLogger.Log($"BluetoothServiceStateApi.DisconnectAudioDevice: anySuccess={anySuccess}");
        return anySuccess;
    }
    ...
    {
        DebugLogger.Log($"BluetoothServiceStateApi.ConnectAudioDevice: MAC={bluetoothAddress:X12}");
        ...
        DebugLogger.Log($"BluetoothServiceStateApi: A2DP enable result={resultA2dp}");
        if (resultA2dp == 0) anySuccess = true;
        ...
        DebugLogger.Log($"BluetoothServiceStateApi: HFP enable result={resultHfp}");
        if (resultHfp == 0) anySuccess = true;

        // 3. Also try HSP
        Guid hspGuid = HeadsetGuid;
        uint resultHsp = BluetoothSetServiceState(IntPtr.Zero, ref addr, ref hspGuid, BLUETOOTH_SERVICE_ENABLE);
        DebugLogger.Log($"BluetoothServiceStateApi: HSP enable result={resultHsp}");
        if (resultHsp == 0) anySuccess = true;

        DebugLogger.Log($"BluetoothServiceStateApi.ConnectAudioDevice: anySuccess={anySuccess}");
        return anySuccess;
    }
}
